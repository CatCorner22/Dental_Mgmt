...
| Functions the UI and UX review (`docs/16`) introduced, audited by hand | 128 |
...
Counts of the rows in the Every function table: 843 rows for the 837 live functions (a name that occurs twice in one file — `onInput` in palette, perio and roles, `onBlur` in roles — has one row per handler; `paint` and `onClick` in shell share a row) plus the eight rows kept for functions the review's third wave retired, so their history stays readable.
...
Rows the UX review's third wave (2026-09-13) retired, because the function no longer exists; each is replaced by a shared builder or by the gate-on-press rule: `screens/checkout.js:field` (the screen's private field builder, replaced by `ui.js:field` — c038535), `screens/palette.js:onBlur` and `screens/phone.js:onBlur` (validation on leaving a field, removed so that a value is judged only on the press — cd2eac9, INT-keep-data-and-gate-on-press), `screens/perio.js:flash` (replaced by `screens/perio.js:say`), `screens/perio.js:validateTag` (replaced by `toothInvalid`/`textInvalid` and `clearOnBlur`) and `screens/perio.js:key` (the pad-key builder, replaced by `padKey` and `kbd`) — all 4de0499, `screens/roles.js:saveButton` (replaced by `saveRow`, which keeps Cancel beside the primary) and `screens/roles.js:mark` (blur-time marking, replaced by `refreshSave` and `liveErrors` on the press) — 3979830.

Functions the UI and UX review (`docs/16`) introduced, which none of the earlier passes could have seen. Thirty came with the shared layer and the Encounter and carry rows above this line in the table below; the ninety-eight below them came with the nine screen fixes of the review's third wave. Each row's Reached by column names the control or render path that calls the function, and its Evidence column names the wave-3 measurement (the screen agent's before/after probe, recorded in `knowledge/reviews/ux-review/findings.md`), the regression check (`A-ux-1..8`) or the probe run on the final tree that exercised it; where a sentence comes from reading the code rather than from a probe, the row says so: `screens/board.js:shortcutsOn`, `screens/board.js:withKey`, `screens/board.js:withKeyIf`, `screens/board.js:keyTargets`, `screens/board.js:markUndo`, `screens/board.js:metaWords`, `screens/chairs.js:shortcutsOn`, `screens/chairs.js:mark`, `screens/chairs.js:flag`, `screens/chairs.js:printKey`, `screens/chairs.js:wrapText`, `screens/checkout.js:caretEnd`, `screens/checkout.js:confirmPending`, `screens/checkout.js:fieldGate`, `screens/checkout.js:precheck`, `screens/checkout.js:readback`, `screens/checkout.js:th`, `screens/checkout.js:labelledRow`, `screens/checkout.js:back`, `screens/dailyclose.js:clearWords`, `screens/dailyclose.js:rowWords`, `screens/dailyclose.js:keysAllowed`, `screens/dailyclose.js:rowChip`, `screens/dailyclose.js:gradeMark`, `screens/dailyclose.js:warnGlyph`, `screens/dailyclose.js:gateFocus`, `screens/dailyclose.js:escapeConfirm`, `screens/dailyclose.js:tileOpen`, `screens/dailyclose.js:locOpenFor`, `screens/dailyclose.js:tenderTable`, `screens/dailyclose.js:stamp`, `screens/dailyclose.js:tid`, `screens/dailyclose.js:onConfirm`, `screens/dailyclose.js:cancel`, `screens/dailyclose.js:pinGate`, `screens/dailyclose.js:whyBlock`, `screens/moneydesk.js:sum`, `screens/moneydesk.js:trace`, `screens/moneydesk.js:claimLine`, `screens/moneydesk.js:shortcutsOn`, `screens/moneydesk.js:keyCap`, `screens/moneydesk.js:confirmStep`, `screens/moneydesk.js:mark`, `screens/moneydesk.js:keyed`, `screens/moneydesk.js:apply`, `screens/moneydesk.js:formErrors`, `screens/moneydesk.js:refuseWriteoff`, `screens/moneydesk.js:doSend`, `screens/moneydesk.js:sendStatement`, `screens/moneydesk.js:plainReason`, `screens/palette.js:onKeydown`, `screens/palette.js:clearDobError`, `screens/palette.js:refuseDob`, `screens/perio.js:say`, `screens/perio.js:shortcutsOn`, `screens/perio.js:inWorkSurface`, `screens/perio.js:gateNode`, `screens/perio.js:confirmable`, `screens/perio.js:paint`, `screens/perio.js:textInvalid`, `screens/perio.js:clearOnBlur`, `screens/perio.js:kbd`, `screens/perio.js:padKey`, `screens/perio.js:groupLabel`, `screens/perio.js:onInput`, `screens/perio.js:onInput`, `screens/perio.js:obsBtn`, `screens/phone.js:paintShow`, `screens/phone.js:toggleShow`, `screens/phone.js:onConfirm`, `screens/rail.js:precheckStatement`, `screens/roles.js:orList`, `screens/roles.js:granters`, `screens/roles.js:canIssue`, `screens/roles.js:seatWord`, `screens/roles.js:cannotLine`, `screens/roles.js:saveKeyOf`, `screens/roles.js:refreshSave`, `screens/roles.js:liveErrors`, `screens/roles.js:heldWhy`, `screens/roles.js:saveRow`, `screens/roles.js:closeForm`, `screens/shell.js:paintShow`, `screens/shell.js:toggleShow`, `screens/shell.js:lockLeft`, `screens/shell.js:write`, `screens/shell.js:type`, `screens/shell.js:clear`, `screens/signin.js:osTheme`, `screens/signin.js:themeIsSystem`, `screens/signin.js:useSystemTheme`, `screens/signin.js:afterHours`, `screens/signin.js:tile`, `screens/signin.js:seg`, `screens/signin.js:heading`, `screens/signin.js:row`, `screens/signin.js:rowsWrap`, `screens/signin.js:fill`.

Functions the post-merge guard restoration (2026-09-17, `scripts/beta/audit/regress-3.mjs`) introduced, after a store rewrite that landed behind the adversarial swarm dropped guards the swarm had pinned: `store.js:openDenial` (the account's denied claim with no appeal sent, read by `evaluateRelease` so a write-off on such an account routes through dual release at any amount — A-regress-3-3, S-moneydesk-close-10) and `screens/perio.js:uncoded` (the count of screening sextants still empty, so the headline names the gap instead of reading "all six coded" at the cursor's end — S-perio-7).
...
| `uncoded` | `screens/perio.js` | — | operational | S-perio-7 | Sextants still empty in the screening lane; with three coded and the cursor past the last, the headline reads "go back to code the 3 empty", not "All six sextants coded" |
...
| `lowerFirst` | `screens/encounter.js` | — | operational | Sentence-casing a verb that follows other words in a read-back or a gate line. | A-ux-8: 'File the note' inside a read-back reads 'file the note'; a word that is already lower case, an empty string and a single character each come back unchanged. |
| `applyGateClearance` | `screens/encounter.js` | — | operational | Every Encounter render, after the filing gate is laid out. | A-ux-8: the screen's root carries --gate-clearance equal to the gate's height plus 8px, and the canvas reads it as scroll-padding-bottom, so 0 focused elements are covered at 1280, 1024 and 420 px, at rest and with the filing checks standing. |
| `clearGateClearance` | `screens/encounter.js` | — | operational | Leaving the Encounter, and any render with no gate standing. | A-ux-8: --gate-clearance is removed, so no other screen inherits a bottom scroll padding it does not need. |
| `watchGate` | `screens/encounter.js` | — | operational | applyGateClearance, through a ResizeObserver on the gate. | A-ux-8: growing the gate by adding a filing check raises the clearance in the same frame; the observer is disconnected when the gate leaves. |
| `bigGlyph` | `screens/encounter.js` | — | operational | The filing gate's severity mark and the read-back's warning. | A-ux-8: the mark computes at the glyph-large token rather than the 11px literal it replaced. |
| `warnMark` | `screens/encounter.js` | — | operational | The read-back before File and before a note starter overwrites typed words. | A-ux-8: the warning sits beside bold text and is not the only signal of severity. |
...
| `shortcutsOn` | `screens/board.js` | — | operational | The Board's document keydown handler, keyTargets and withKeyIf, on every render and every key. | Wave 3 CUST-2.1.4 probe: with the shipped preference (off) the keys a, s and c leave the store byte-identical (75,986 → 75,986 chars), no appointment arrives and the hash is unchanged; with `settings.shortcuts.on` the same keys act. A-ux-6 holds the preference to seven with a reset. |
| `withKey` | `screens/board.js` | — | operational | The Arrive, Seat and Check out controls a key would fire, while shortcuts are on. | Wave 3 CLT-recognition-keys probe: with shortcuts on, the controls carry `aria-keyshortcuts` and a `<kbd>` inside the button; the page sub-line names no key ('8:40 am · Thursday 9/3 · 3 chairs · 25 appointments'), so 0 shortcuts live only in a legend. |
| `withKeyIf` | `screens/board.js` | — | operational | Every card's action row; passes the button through unchanged when the key does not target that card. | Same probe: exactly one card per key carries the keycap (2 controls at rest); with shortcuts off, 0 `<kbd>` and 0 `aria-keyshortcuts` in the canvas. |
| `keyTargets` | `screens/board.js` | — | operational | Once per render before the cards paint; the result is held in keyMarks. | Same probe: the first arrivable, the first arrived and the first checkout-able appointment in time order are the marked cards; returns `{}` while shortcuts are off, so no card is marked. |
| `markUndo` | `screens/board.js` | — | operational | After Call lab and Sign out the tablet on the readiness strip, so the strip's undo row names what it would take back. | Board probe on the final tree: pressing `board.readiness.row.lab-op3.call` announces 'Called Ridge Dental Lab at 8:40 am — marked on your readiness strip. Undo sits beside it.' and renders the row 'Called Ridge Dental Lab · 8:40 am' with `board.readiness.row.lab-op3.undo` ('Undo the lab call'); pressing it announces 'Lab call taken back — the row is back on the strip.', the Call lab row returns and 0 undo rows remain. |
| `metaWords` | `screens/board.js` | — | operational | Every appointment card's meta line. | Wave 3 CLT-chip-vocab probe: the type, eligibility, alert count, lab case and referral read as one dotted line, leaving 31 visible chips of 10 distinct labels and at most one chip per card face. |
| `shortcutsOn` | `screens/chairs.js` | — | operational | The Chairs document keydown handler and printKey, on every render and key. | Wave 3 CUST-2.1.4 probe: with the default preference, p, n and r leave the hash at #/hygienist/chairs, focus on body and nothing written; with the preference on, the same keys open perio, the note and Ready for exam. |
| `mark` | `screens/chairs.js` | — | operational | flag(), for every since-last-visit line and alert. | Wave 3 CDS-WARNING probe: the glyph is `aria-hidden` and sits beside bold text, so severity is never carried by the mark alone. |
| `flag` | `screens/chairs.js` | — | operational | The since-last-visit strip and the anticoagulant alert on each card. | Wave 3 CLT-chip-vocab and CLT-redundancy probes: 9 visible chips, a vocabulary of 6, at most 2 chips per card, and the anticoagulant text appears once (on a-1042) rather than on every card. |
| `printKey` | `screens/chairs.js` | — | operational | The Chart perio, Write note and Ready for exam controls of the one card each key targets, while shortcuts are on. | Wave 3 CLT-recognition-keys probe: shortcuts on → `<kbd>` P, N, R inside `chairs.card.a-1042.perio/.note/.ready` with `aria-keyshortcuts`; shortcuts off → 0 keys anywhere and the sub-line reads '8:40 am · 6 chairs · yours'. |
| `wrapText` | `screens/chairs.js` | — | operational | Every action button on a Chairs card. | Wave 3 CDS-BTN probe: 0 labels wrapping at 1024 or 1280, and at 320 px the reflow probe finds 0 px of horizontal overflow because a long label wraps inside its 44 px-tall button instead of widening the row. |
| `caretEnd` | `screens/checkout.js` | — | operational | After the rerender an edit forces while a refusal or a Post read-back stands, on the amount, card-number and write-off fields. | Checkout probe on the final tree: with the Post read-back standing and the amount at 44.00, typing 5 drops the read-back and leaves the value 44.005 with the caret at 6 of 6 and the field focused; typing 7 gives 44.0057 with the caret at 7 of 7. |
| `confirmPending` | `screens/checkout.js` | — | operational | Every edit to the payment form while a Post read-back stands. | Wave 3 INT-irreversible-identity probe: the first Post press writes 119 → 119 ledger rows and renders `checkout.post.confirm`/`.cancel`; any edit to the form drops the read-back, so a changed amount is never posted under an old read-back. |
| `fieldGate` | `screens/checkout.js` | — | operational | precheck(), when a tender or an amount is missing. | Wave 3 CDS-ERR probes: after Post with a blank amount the field carries `aria-invalid="true"`, the message stands before the input at weight 600, and one `role="alert"` summary with one link ('Amount: type an amount above $0.00') moves focus to `#co-amount`. |
| `precheck` | `screens/checkout.js` | — | operational | The first Post press, before the read-back is offered. | Same probes: Post with no tender raises 'Choose a tender' whose control selects card; Post with $0 raises 'Type an amount above zero'; a complete form passes to the read-back and nothing is written. |
| `readback` | `screens/checkout.js` | — | operational | The Post read-back row's sentence. | Wave 3 INT-irreversible-identity probe: the row names the amount, the tender, any write-off with its reason, the patient, and whether the note is still unfiled; A-ux-4 finds Cancel focused and equal in size. |
| `th` | `screens/checkout.js` | — | operational | The completed-procedures table's header cells. | Wave 3 AXE-1.3.1 probe: 7 `th`, 7 with `scope`, 0 `thead th` without scope, a caption; axe table rules report 0 violations. |
| `labelledRow` | `screens/checkout.js` | — | operational | The decision, tender and write-off groups of the Payment region. | Wave 3 CLT-common-region probe: 0 groups of three or more controls without a visible label on Checkout at rest and with the write-off open; the largest bounded group at rest is 3. |
| `back` | `screens/checkout.js` | — | operational | The finish row, beside Post, in the ordinary and the held states. | Wave 3 CLT-neutral-irreversible probe: `checkout.post`'s row holds `checkout.back` (44 px, `.btn.reversible`) before it in every state, so the irreversible verb is never alone in its row. |
| `clearWords` | `screens/dailyclose.js` | — | operational | The Clear read-back and the cleared stamp. | Wave 3 INT-real-world-words probe: 0 store-key-shaped tokens on the glass; the reason prints as its words ('duplicate posting'), never its code. |
| `rowWords` | `screens/dailyclose.js` | — | operational | Every sentence that names a ledger row. | Same probe: 'ledger row 4504' on the glass, 0 matches for `le-nnnn`-shaped keys. |
| `keysAllowed` | `screens/dailyclose.js` | — | operational | The T key on the tied tile. | Wave 3 CUST-2.1.4 probe: shortcuts off (default) → 't' and 'T' leave `close.tied.tile` `aria-expanded` unchanged and the store at 75,986 chars; on → the key toggles the tile and the `<kbd>` is printed on it. |
| `rowChip` | `screens/dailyclose.js` | — | operational | gradeMark(), for the review and variance grades. | Wave 3 CLT-chip-not-button probe: 0 interactive chips; the chip's glyph is set at the shared `--glyph` size and inherits the chip's ink. |
| `gradeMark` | `screens/dailyclose.js` | — | operational | The tile, every location card and every reconciliation row. | Wave 3 CDS-STATUS and COLOR probes: 'Tied' renders as a shape and a word on the card's own ground, so the severity fills stay at 2 distinct fills in the first screenful. |
| `warnGlyph` | `screens/dailyclose.js` | — | operational | The Close-day question's heading. | Wave 3 CDS-WARNING probe: `#dc-close-confirm` glyph computes 28 px in a 22 × 33 box beside a weight-600 heading. |
| `gateFocus` | `screens/dailyclose.js` | — | operational | Every refused posting on the shared desk, to choose where focus lands. | Wave 3 CDS-ERR-summary probe: a refused PIN focuses `close.pin.errors` (the summary before the field); any other refusal focuses `refusal.control`. |
| `escapeConfirm` | `screens/dailyclose.js` | — | operational | Escape while a Match, Clear, Retire or Close-day read-back stands. | Wave 3 INT-exit-and-undo probe: `close.closeday.confirm` present before Escape, absent after, and the same handler cancels a pending Match, Clear or Retire; focus returns to the verb that raised it. |
| `tileOpen` | `screens/dailyclose.js` | — | operational | tile(), on every render. | Wave 3 CLT-disclosure-depth probe: the tile is `aria-expanded="true"` at load whenever the day is not tied, so the variance's controls and '312.40' are on the glass without a press. |
| `locOpenFor` | `screens/dailyclose.js` | — | operational | Every location card, on every render. | Same probe: the first location in variance-first order opens by itself; a tied location stays closed until pressed. |
| `tenderTable` | `screens/dailyclose.js` | — | operational | Each location's tender table and the Close-day totals. | Wave 3 AXE-1.3.1 probe: 2 tables, each with a caption, 6 `th` all `scope="col"`; A-ux-1 style: the wrapper is a `scrollRegion`, in the tab order only while the table overflows. |
| `stamp` | `screens/dailyclose.js` | — | operational | After Match and Clear on a variance card. | Wave 3 INT-success-says-what-next probe: 4 of 4 commits confirmed in text with a next step, each announcement present verbatim on the glass. |
| `tid` | `screens/dailyclose.js` | — | operational | The three decision controls of every raised-threshold card. | CONTRACTS §4 `close.decision.<id>.<keep|tighten|retire>`; proto-check `testids` PASS on the final tree. |
| `onConfirm` | `screens/dailyclose.js` | — | operational | The second press of Retire. | Wave 3 INT-irreversible-identity probe: the first Retire press renders `close.decision.<id>.retire.confirm`/`.cancel` and writes nothing; only this callback reviews the decision. |
| `cancel` | `screens/dailyclose.js` | — | operational | The Close-day read-back's Cancel and its Escape path. | Same probe on Close day: Cancel returns `closeStep` to idle and focus to `close.closeday` with the day still open. |
| `pinGate` | `screens/dailyclose.js` | — | operational | pinGates() and the PIN field's error line, on every render of the shared desk. | Wave 3 CDS-ERR-message probe: exactly 1 `aria-invalid` element in the canvas after a refused PIN, and `#dc-pin-err` reads 'Error: Enter your PIN to post' before the input. |
| `whyBlock` | `screens/dailyclose.js` | — | operational | Every row of the Risk register. | Wave 3 CLT-prose-budget probe on Risk: 64 words above the fold with the Why text folded; the disclosure toggles `hidden` on the body. |
| `sum` | `screens/moneydesk.js` | — | operational | Batch totals on the ERA tab and the write-off cap. | Wave 3 INT-irreversible probe: the Post-matched read-back states '37 ledger rows worth $20,423.00', the sum of the matched lines. |
| `trace` | `screens/moneydesk.js` | — | operational | The ERA batch heading. | Wave 3 INT-real-world-words probe: 0 raw store keys on the seven tabs; the batch prints 'payer trace 20260903-90112', not `era-1` or 'TRN …'. |
| `claimLine` | `screens/moneydesk.js` | — | operational | Every claim row on the aging, denials and ERA tabs. | Same probe: the row reads the procedure, tooth and payer in words. |
| `shortcutsOn` | `screens/moneydesk.js` | — | operational | keyCap, keyed and the document keydown handler. | Wave 3 CUST-2.1.4 probe: with shortcuts off, p, w and a produce 0 actions (only the raw keydown log), the selected tab stays `money.tab.era`, no write-off form opens. |
| `keyCap` | `screens/moneydesk.js` | — | operational | The Post matched, Write off and Appeal controls. | Wave 3 CLT-recognition-keys probe: with shortcuts on, 2 `<kbd>` on the ERA tab (P, W) at 14 px plus A on Denials, each inside its own button; the legend is gone. |
| `confirmStep` | `screens/moneydesk.js` | — | operational | Every irreversible posting verb on the Money Desk. | Wave 3 CDS-WARNING probe: the read-back's mark is 32 × 32 px at 28 px font beside the verb at weight 600; A-ux-4 finds Cancel focused. |
| `mark` | `screens/moneydesk.js` | — | operational | confirmStep, on the frame after the first press. | Same probe: measured after the click, the `.confirmrow .glyph` carries the MARK style. |
| `keyed` | `screens/moneydesk.js` | — | operational | The three keyed controls, and again after Cancel rebuilds the first button. | Wave 3 CLT-recognition-keys probe: `aria-keyshortcuts` present on the control after a Cancel round trip. |
| `apply` | `screens/moneydesk.js` | — | operational | keyed(), at build and after each click. | Same probe. |
| `formErrors` | `screens/moneydesk.js` | — | operational | The Post press on the write-off form. | Wave 3 CDS-ERR-summary probe: 1 `role="alert"` with a heading and 2 links (amount, reason) before the form. |
| `refuseWriteoff` | `screens/moneydesk.js` | — | operational | postWriteoff(), when formErrors is non-empty or the store refuses. | Same probe: `document.activeElement` is `money.writeoff.errors` after the press; WCAG 3.3.1 probe: `aria-describedby` reads 'Error: Enter an amount above zero, like 410.00'. |
| `doSend` | `screens/moneydesk.js` | — | operational | The Send appeal read-back's confirm. | Wave 3 INT-irreversible probe: the first Send press writes nothing; the confirm sends and the stamp stays on the row. |
| `sendStatement` | `screens/moneydesk.js` | — | operational | The Send statement read-back's confirm on the Statements tab. | Wave 3 INT-success probe: 'The statement went out — watch the ledger for the payment' announced and stamped on the row. |
...
| `gateNode` | `screens/perio.js` | — | operational | Every refusal the perio screen raises. | Wave 3 CDS-WARNING probe: `.refusal .glyph` computes 24 px in a 24 × 24 box; the verb line stays weight 600. |
| `confirmable` | `screens/perio.js` | — | operational | Save exam and Save tag. | A-ux-4 and the perio flow (3 taps incl. `perio.save.confirm`): the read-back's glyph is repainted at the large size whenever the row is rebuilt. |
| `paint` | `screens/perio.js` | — | operational | confirmable(), through a MutationObserver on the slot. | Same probe: the glyph style is present after the first press and again after Cancel and a second press. |
| `textInvalid` | `screens/perio.js` | — | operational | Save tag and clearOnBlur. | Wave 3 WCAG-3.3.1 probe: an empty observation yields 'Error: Observation: say what you saw.' on the press only. |
| `clearOnBlur` | `screens/perio.js` | — | operational | Leaving the Tooth or Observation field. | Wave 3 INT-keep-data probe: Tab out of an empty field leaves `aria-invalid` null; a standing error is removed in place once the value is valid, and the field is not detached. |
| `kbd` | `screens/perio.js` | — | operational | padKey(), for caps whose keystroke differs from the label. | Wave 3 CLT-recognition-keys probe: 17 `aria-keyshortcuts` with the pad open and the keycaps print 'Space' for the skip key. |
| `padKey` | `screens/perio.js` | — | operational | The 15 glove-pad keys. | A-ux-3: every pad key's accessible name leads with its printed label through `leadWithLabel`; 0 label-content-name mismatches. |
| `groupLabel` | `screens/perio.js` | — | operational | The depth, marks and move groups of the glove pad. | Wave 3 CLT-common-region probe: 0 unlabelled groups with the pad open; each group's name is visible inside its bounded region. |
| `onInput` | `screens/perio.js` | — | operational | Typing in the Tooth field (the first of two handlers of this name). | Same probes: the typed value survives a refusal and a rerender. |
| `onInput` | `screens/perio.js` | — | operational | Typing in the Observation field (the second handler of this name). | Same probes. |
| `obsBtn` | `screens/perio.js` | — | operational | The common-observation words above the tag form. | Wave 3 CLT-chunk-4 probe: two named rows of 3 rather than one row of 11; pressing a word writes it into the Observation field. |
| `paintShow` | `screens/phone.js` | — | operational | The step-up PIN pad, at build and after each toggle. | Wave 3 CUST-3.3.8 probe: three keys show '•••'; pressing the show control gives '123' with `aria-pressed="true"`, pressing again gives '•••'. |
| `toggleShow` | `screens/phone.js` | — | operational | The Show digits control. | Same probe; focus stays on the control after the repaint. |
| `onConfirm` | `screens/phone.js` | — | operational | The Send back read-back's confirm on a request card. | Wave 3 INT-irreversible probe: the first press renders '▲ This cannot be undone: the request goes back…' and declines only on the confirm. |
| `precheckStatement` | `screens/rail.js` | — | operational | The first Send statement press on the Ledger. | Wave 3 WCAG-3.2.4 probe: `ledger.statement.send` under a gate reads visible 'Held', aria-label 'Held: Send statement'. From the code, not a probe: on a shared desk with no PIN it raises 'Enter your PIN to send' before any read-back, and under an outage it asks the store first so its refusal shows in the store's words. |
| `orList` | `screens/roles.js` | — | operational | granters() and cannotLine(). | Wave 3 INT-instructions probe: 'Only a seat that grants roles can issue a day pass…ask Dr. Reagan or …' on the glass for a seat that cannot issue. |
| `granters` | `screens/roles.js` | — | operational | cannotLine(), on every render for a seat without `grant_roles`. | Same probe. |
| `canIssue` | `screens/roles.js` | — | operational | heldWhy() and the day-pass form's Add control. | Wave 3 INT-no-disabled probe: 0 `button[disabled]`; a seat that cannot issue sees the primary Held with the reason 'your seat does not grant roles'. |
| `seatWord` | `screens/roles.js` | — | operational | cannotLine(). | Same probe: the seat is named by its role label, never its store key. |
| `cannotLine` | `screens/roles.js` | — | operational | The Roles landing for a non-granting seat. | Same probe; CLT-reading-grade: longest sentence 15 words. |
| `saveKeyOf` | `screens/roles.js` | — | operational | refreshSave(), after every field edit. | Wave 3 INT-no-disabled probe: tabbing out of the empty name field leaves the primary as 'Issue day pass' and marks the field instead. |
| `refreshSave` | `screens/roles.js` | — | operational | Every input and blur on the day-pass form. | Same probe: the save row is replaced only when its key changes and never while a read-back stands. |
| `liveErrors` | `screens/roles.js` | — | operational | heldWhy(), on every render. | Same probe: an error is live only while its field is still invalid. |
| `heldWhy` | `screens/roles.js` | — | operational | saveRow(), on every render. | Wave 3 WCAG-3.2.4 probe: in every Held state the primary reads visible 'Held' with `aria-label` 'Held: <reason>'. |
| `saveRow` | `screens/roles.js` | — | operational | The day-pass form's finish row. | Wave 3 CLT-neutral-irreversible probe: Cancel (96 × 44, reversible) and Issue day pass (187 × 44, irreversible) on the same row top, the primary not focused on load. |
| `closeForm` | `screens/roles.js` | — | operational | Cancel on the day-pass form. | Same probe: the form closes, its state resets and focus returns to `roles.daypass.add`. |
| `paintShow` | `screens/shell.js` | — | operational | The author PIN pad, at build and after each toggle. | Wave 3 CUST-3.3.8 probe: `pin.show` flips '•••' to '123' and back with `aria-pressed`; no dialog opens and no auth event is written by the preference. |
| `toggleShow` | `screens/shell.js` | — | operational | The Show digits control on the author pad. | Same probe; the control keeps focus after the repaint. |
| `lockLeft` | `screens/shell.js` | — | operational | The author pad after three misses. | Wave 3 WCAG-2.2.1 probe: 'Time left on this device: 5:00.' reads 4:59 1.6 s later and is replaced by 'The lock has ended — enter the PIN again.' |
| `write` | `screens/shell.js` | — | operational | lockLeft(), once a second. | Same probe. |
| `type` | `screens/shell.js` | — | operational | The twelve keypad keys of the author pad. | Wave 3 CLT-common-region probe: the keypad is `role="group"` labelled by the visible 'Keypad'; a fourth digit past six is refused. |
| `clear` | `screens/shell.js` | — | operational | pulseFor(): bound to the pointed control's click and to hashchange, and stored so a second point-at can clear the first. | Shell probe on the final tree: after `rail1.chip.0` the ring (`.pointed`) sits on `board.card.a-1042.arrive` at 0 s and is still there at 7 s; pressing that control leaves 0 `.pointed` elements. |
| `osTheme` | `screens/signin.js` | — | operational | useSystemTheme(), on Reset to defaults. | Sign-in probe with the OS dark (colorScheme: dark): after Light then `signin.reset`, `P.theme` is 'dark', `P.themePref` 'system' and `<html>` carries no `data-theme`. |
| `themeIsSystem` | `screens/signin.js` | — | operational | The theme segment's pressed states and the '· System default' mark on its heading. | Same probe: at rest and after Reset, Light and Dark both read `aria-pressed="false"` and the heading carries 'System default'; after Light, Light reads true and the mark is gone. |
| `useSystemTheme` | `screens/signin.js` | — | operational | Reset to defaults (`signin.reset`). | Same probe: `data-theme` removed, `P.themePref` 'system', `P.theme` back to the OS's 'dark'. |
| `afterHours` | `screens/signin.js` | — | operational | Whether the test harness opens by itself, and the After hours option's pressed state. | Sign-in probe: `signin.afterhours` reads `aria-pressed` false → true on press, `Proto.store.get().clock.afterHours` is true, and on reload the harness stands open without a press. |
| `tile` | `screens/signin.js` | — | operational | The nine role tiles. | Wave 3 CLT-label-words probe: each button reads its role alone ('Hygienist') with the landing line joined by `aria-describedby`; 0 of 16 visible labels over 4 words. |
| `seg` | `screens/signin.js` | — | operational | The theme, device and simulation segments. | Wave 3 CLT-common-region probe: 0 unlabelled groups; every segment is `role="group"` labelled by its visible heading. |
| `heading` | `screens/signin.js` | — | operational | seg() labels. | Same probe: 'Colour scheme · System default' marks the system choice at regular weight. |
| `row` | `screens/signin.js` | — | operational | Every preference row on sign-in. | Same probe. |
| `rowsWrap` | `screens/signin.js` | — | operational | The 'Before you start' region and the harness. | Wave 3 INT-temp-first-screenful probe: 14 controls at 1280 × 800 and 11 at 420 × 860, the rows wrapping rather than overflowing. |
| `fill` | `screens/signin.js` | — | operational | The test-harness disclosure body, on open and after each choice. | Wave 3 CLT-hick-7 probe: Device profile 4, Aids and simulations 3; no group over 7. |
