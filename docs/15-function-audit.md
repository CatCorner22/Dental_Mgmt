...
| Measure | Count |
|---|---|
| Functions in `prototype/js` (`scripts/audit/inventory.mjs`) | 752 |
...
Functions the SuperByte round (2026-09-11) introduced in the new module `screens/superbyte.js`, which no earlier audit could have seen. Each carries a row below whose Reached by column names the check(s) in `scripts/beta/audit/superbyte.mjs` (U-sb-1 … U-sb-10) or the screen path that drives it: `standardize`, `atSentenceStart`, `completeness`, `coreCoverage`, `clamp01`, `direction`, `isSurface`, `measure`, `quote`, `instrument`, `push`, `verify`, `phiGate`, `hit`, `S`, `viewFor`, `dentistLike`, `draftOf`, `filedOf`, `noteText`, `rerender`, `focusTid`, `renderPicker`, `render`, `refresh`, `noteCard`, `field`, `onInput`, `structurePanels`, `wordingCard`, `applyWording`, `MOOD`, `face`, `R`, `compass`, `el`, `rails`, `bytePanel`, `onCopy` (39; 713 → 752).
...
| `standardize` | `screens/superbyte.js` | 67 | operational | U-sb-5 (unit and Apply paths), wordingCard preview on every keystroke | Language-only rewrites (x-ray, Pt, w/, BWX, qd, naked and trailing decimals, abcess, #30DO) with sentence case kept and the sentence's full stop untouched; `only` set lets Apply skip rows kept as typed; flags vague phrases, ambiguous PA/BW/LA, do-not-use marks and medication misspellings without touching them |
| `atSentenceStart` | `screens/superbyte.js` | 70 | operational | standardize, on every rewrite | "BWX taken." opens a sentence and comes back "Bitewing radiographs taken."; mid-sentence rewrites stay lowercase (U-sb-5 shape) |
| `completeness` | `screens/superbyte.js` | 101 | operational | structurePanels "A later reader will ask" card; U-sb-1 render | Eleven ported Smile Notes rules (trigger/satisfiedBy); returns the first three open rows as question + how-to-satisfy |
| `coreCoverage` | `screens/superbyte.js` | 114 | operational | structurePanels Universal Core card; U-sb-1 render | Eight Universal Core sections marked present/open by presence of their cues; "a blank never means normal" |
| `clamp01` | `screens/superbyte.js` | 128 | operational | measure | Bounds a rail share to [0, 1] before it becomes a percentage |
| `direction` | `screens/superbyte.js` | 129 | operational | measure, every rail | on-target / toward / away against a target or a band (density 8–60, target 20) |
| `isSurface` | `screens/superbyte.js` | 130 | operational | measure fact count | Treats a surface token (DO, MOD, …) as one fact, not shorthand the parser cannot read |
| `measure` | `screens/superbyte.js` | 131 | operational | bytePanel on every keystroke; U-sb-10 rails count | Five NorthStar rails (read coverage .9, active voice .85, pillars 1, TN cues .8, density band) plus onCourse share the compass needle reads |
| `quote` | `screens/superbyte.js` | 161 | operational | instrument evidence | Trims a matched clause to a short quotation for the Source line |
| `instrument` | `screens/superbyte.js` | 162 | operational | bytePanel once the draft passes 24 characters; U-sb-3, U-sb-6 | At most three readings in the fixed order tn-cues → active voice → pillars → read → density, each with say / why / question / source and evidence or a verified-absent cue |
| `push` | `screens/superbyte.js` | 164 | operational | instrument | Adds a reading only while fewer than MAX_READINGS stand |
| `verify` | `screens/superbyte.js` | 177 | operational | bytePanel after instrument; U-sb-6 | Drops any reading whose evidence quotation is not in the masked draft, so nothing is said the text does not carry |
| `phiGate` | `screens/superbyte.js` | 181 | operational | bytePanel before measure and instrument; U-sb-6 | Masks patient and guardian name parts, phone, SSN, email, MRN and calendar dates; returns per-kind counts ("name ×2") and the total the PHI-gate stage prints; no reading or panel text carried Brandt / DOB / phone (U-sb-6) |
| `hit` | `screens/superbyte.js` | 183 | operational | phiGate per shape | Counts and replaces one PHI shape, accumulating the kind's count |
| `S` | `screens/superbyte.js` | 196 | operational | filedOf, renderPicker, viewFor | The store snapshot getter |
| `viewFor` | `screens/superbyte.js` | 197 | operational | render, refresh, bytePanel, wordingCard; U-sb-3, U-sb-8 | Per encounter-and-author view state (paced reading index, rows kept as typed), reset when the store snapshot changes |
| `dentistLike` | `screens/superbyte.js` | 198 | operational | noteCard readonly, wordingCard Apply; U-sb-8, U-sb-10 | Only dentist, owner and surgeon authors edit or apply; the hygienist's fields are read-only and carry no Apply |
| `draftOf` | `screens/superbyte.js` | 199 | operational | noteText, applyWording, noteCard; U-sb-4, U-sb-8 | The Encounter's own per-author draft; typing on the glass shows on the Encounter and a PIN switch shows the new author's draft, not the last one's |
| `filedOf` | `screens/superbyte.js` | 200 | operational | render; U-sb-7 | The latest filed note for the encounter, or null |
| `noteText` | `screens/superbyte.js` | 201 | operational | structurePanels, bytePanel | Filed markdown when filed, otherwise assessment and plan joined |
| `rerender` | `screens/superbyte.js` | 202 | operational | applyWording, keep toggles, picker | Re-renders the current route through Proto.screens.superbyte.render |
| `focusTid` | `screens/superbyte.js` | 203 | operational | pager, Apply, killer control; U-sb-3 focus check | Refocuses a control by test id after a repaint; Previous keeps focus (U-sb-3) |
| `renderPicker` | `screens/superbyte.js` | 206 | operational | #/dentist/superbyte (no id); proto-check ROUTES | One row per loc-1 appointment with an encounter, privacy-aware names, Open buttons superbyte.row.<enc>.open |
| `render` | `screens/superbyte.js` | 223 | operational | Proto.router.on('superbyte'); every U-sb check; proto-check ROUTES | Picker without an id, Nothing here + notfound.home for an unknown id (U-sb-9), otherwise the three-column glass sized to the canvas: no page or column scroll at 1024×768, 1280×800, 1366×768, 1920×1080 (U-sb-1) |
| `refresh` | `screens/superbyte.js` | 245 | operational | onInput per keystroke, pager, keep toggles | Replaces only the wording, structure and Byte panels so the caret field is never rebuilt under the author |
| `noteCard` | `screens/superbyte.js` | 251 | operational | render; U-sb-4, U-sb-7, U-sb-10 | Two textareas bound to the per-author draft (read-only for non-dentists), or the frozen filed text with no fields (U-sb-7) |
| `field` | `screens/superbyte.js` | 253 | operational | noteCard for assessment and plan | Labelled textarea with testid superbyte.note.field.<name> |
| `onInput` | `screens/superbyte.js` | 255 | operational | every keystroke in either field; U-sb-4 | Writes the draft and refreshes the read panels live |
| `structurePanels` | `screens/superbyte.js` | 266 | operational | render, refresh | Before File (the store's own noteKillers rows as refusals, retargeted here), Universal Core and later-reader cards |
| `wordingCard` | `screens/superbyte.js` | 295 | operational | render, refresh; U-sb-5, U-sb-7, U-sb-10 | Three rewrite rows with Keep as typed toggles, flag rows, "+n more", and Apply only for a dentist-like author on a live draft |
| `applyWording` | `screens/superbyte.js` | 314 | operational | superbyte.wording.apply; U-sb-5 | Applies the non-kept rewrites to both fields through standardize(only), recomputes the gate rows if checked, announces the count |
| `MOOD` | `screens/superbyte.js` | 323 | operational | bytePanel | idle / happy / thinking / concerned from onCourse and whether any text stands |
| `face` | `screens/superbyte.js` | 324 | operational | bytePanel head | Pixel SVG built with createElementNS (h() cannot build SVG), aria-hidden, with the observe badge |
| `R` | `screens/superbyte.js` | 327 | operational | face | Places one pixel rectangle |
| `compass` | `screens/superbyte.js` | 339 | operational | bytePanel head | NorthStar compass, needle at −70 + onCourse × 140 degrees, role=img "On course: N percent" beside the printed share |
| `el` | `screens/superbyte.js` | 343 | operational | compass | Creates one namespaced SVG element with attributes |
| `rails` | `screens/superbyte.js` | 358 | operational | bytePanel; U-sb-10 counts five | One line per rail: label, bar with target tick (role=img with the note), direction word; the note stands below only while the rail drifts |
| `bytePanel` | `screens/superbyte.js` | 370 | operational | render, refresh; U-sb-2, U-sb-3, U-sb-6 | Observe-only panel: no inputs, copy/cut/context-menu refused, user-select none (U-sb-2); PHI gate → measure → instrument → verify; status, six-stage pipeline, one paced reading, five rails |
| `onCopy` | `screens/superbyte.js` | 403 | operational | copy event on the panel; U-sb-2 | preventDefault: the pioneer's words never reach the clipboard or the note |
