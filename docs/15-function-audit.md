...
| Measure | Count |
|---|---|
| Functions in `prototype/js` (`scripts/audit/inventory.mjs`) | 835 |
...
Counts of the rows in the Every function table: 841 rows for the 835 live functions (a name that occurs twice in one file — `onInput` in palette, perio and roles, `onBlur` in roles — has one row per handler; `paint` and `onClick` in shell share a row) plus the eight rows kept for functions the review's third wave retired, so their history stays readable.
