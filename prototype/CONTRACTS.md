...
| Encounter | `exams.row.<encId>`, `exams.row.<encId>.open`, `enc.tag.<tagId>.chart`, `enc.tag.<tagId>.dismiss`, `enc.tooth.<1-32>`, `enc.surface.<tooth>.<m|o|d|b|l>`, `enc.proc.<cdt>`, `enc.temporality.<today|planned|existing>`, `enc.note.field.<id>`, `enc.note.starter.<n>`, `enc.note.superbyte`, `enc.killer.<n>.fix`, `enc.readback.switch`, `enc.file`, `enc.undo` |
| SuperByte | `superbyte.row.<encId>.open` (the picker, no id in the route), `superbyte.rail`, `superbyte.back`, `superbyte.note.field.<assessment|plan>`, `superbyte.note.frozen` (a filed note's text, no handler), `superbyte.killer.<n>.fix`, `superbyte.wording.<n>.keep`, `superbyte.wording.apply`, `superbyte.reading.prev`, `superbyte.reading.next`; read-only landmarks the hunters measure: `superbyte.panel`, `superbyte.status`, `superbyte.layer`, `superbyte.phi`, `superbyte.oncourse`, `superbyte.reading` |
