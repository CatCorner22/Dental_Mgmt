 ...
 * every read. These page the owner one at a time, so the route is for the
 * administrator rank; everything else waits for the weekly digest.
 */
export const GET = withGuard(
  async (_req, ctx) => {
    ...
    const items = await withTenantTransaction(user.tenantId, user.id, (db) => listHardEvents(db, user.tenantId, { since, now }));
