/**
 * Ridgeview Family Dental — test fixture only.
...
/** Fresh PracticeState clone for tests. Mutating the result does not leak across cases. */
export function ridgeviewPractice(): PracticeState {
  return {
    people: people.map((p) => ({ ...p })),
    knowledge: knowledge.map((k) => ({
      ...k,
      linkedProcessIds: [...k.linkedProcessIds],
    })),
    ...
    scenarios: scenarios.map((s) => ({
      ...
      statSources: [...s.statSources],
      cascadeLayers: [...s.cascadeLayers],
      mitigations: s.mitigations.map((m) => ({ ...m })),
    })),
    ...
    processes: processes.map((p) => ({
      ...
      ownerPersonIds: p.ownerPersonIds ? [...p.ownerPersonIds] : undefined,
      inputs: p.inputs ? [...p.inputs] : undefined,
      outputs: p.outputs ? [...p.outputs] : undefined,
      risks: p.risks?.map((r) => ({ ...r })),
      ideas: p.ideas?.map((i) => ({ ...i })),
      wastes: p.wastes?.map((w) => ({ ...w })),
    })),
  };
}
