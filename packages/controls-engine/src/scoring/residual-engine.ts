/**
 * Residual risk scoring over PracticeState.
 * Directional until a CPA calibrates the weights. Never scores people —
 * scores control design, knowledge concentration, and scenario residual.
 */
...
export function scoreAllResidualRisks(
  ...
  const staff = aligned.staff;
  const scoped = withStaff(state, staff);
  const risks = findKnowledgeRisks(scoped);
  const knowledgeRedundancy =
    risks.filter((r) => r.ownerCount >= 2).length /
    Math.max(1, risks.length);

  const controlScores = scoped.controls.map((c) =>
    scoreControl(c, staff, knowledgeRedundancy),
  );

  const knowledgeScores = risks.map((r) => {
    const k = scoped.knowledge.find((x) => x.id === r.knowledgeId);
  ...
  const scenarioScores = scoped.scenarios.map((s) => {
    const result = runPrecogScenario(scoped, s.id, {
      staff,
      riskVariables: aligned.vars,
    })!;
    const lossNorm = clamp01(result.financialImpact.expected / 125000);
    // Longer time-to-detection increases residual (detection lag).
    const timeNorm = clamp01(result.timelineDays.p50 / 240);
    ...
    return {
      ...
      drivers: [
        ...
        {
          id: `${s.id}-time`,
          label: "Detection lag / time to detection",
          direction: "increases" as const,
          weight: timeNorm,
          detail: `p50 ${result.timelineDays.p50} days (95% ${result.timelineDays.p95Low}–${result.timelineDays.p95High}) — longer lag increases residual`,
        },
        ...uplift.drivers,
      ].slice(0, 6),
...
export function portfolioSummary(
  ...
  const scores = scoreAllResidualRisks(state, vars);
...
export function tornadoSensitivity(state: PracticeState) {
  const base = portfolioSummary(state).averageResidual;
  ...
  const trials: { id: string; label: string; staff: StaffComposition }[] = [
    {
      ...
      staff: { ...state.staff, dualControlPayments: true },
    },
    {
      id: "bank",
      label: "Independent bank reconciliation",
      staff: { ...state.staff, independentBankRec: true },
    },
    {
      id: "seg",
      label: "Raise segregation score to 75",
      staff: { ...state.staff, segregationScore: 75 },
    },
    {
      id: "spof",
      label: "Eliminate sole-owner knowledge",
      staff: { ...state.staff, soleOwnerKnowledgeCount: 0 },
    },
    {
      id: "team",
      label: "Grow team to 10 (more SoD room)",
      staff: { ...state.staff, teamSize: 10 },
    },
  ];

  for (const t of trials) {
    const improved = portfolioSummary(withStaff(state, t.staff)).averageResidual;
    levers.push({
      id: t.id,
      label: t.label,
      delta: base - improved,
