 ...
 * Multipliers are directional until a CPA calibrates them.
 */
...
/** Apply variable toggles onto a staff copy so residual scoring sees the intervention. */
export function applyVariablesToStaff(
  staff: StaffComposition,
  vars: RiskVariableState,
): StaffComposition {
  return {
    ...staff,
    dualControlPayments: vars.hasDualControl,
    independentBankRec: vars.hasIndependentBankRec,
  };
}

/** Align staff ↔ variable booleans. Variable toggles win when both are present. */
export function alignedStaffAndVariables(
  staff: StaffComposition,
  vars?: RiskVariableState,
): { staff: StaffComposition; vars: RiskVariableState } {
  if (!vars) {
    const nextVars = mergeStaffIntoVariables({ ...DEFAULT_RISK_VARIABLES }, staff);
    return { staff, vars: nextVars };
  }
  const nextStaff = applyVariablesToStaff(staff, vars);
  return {
    staff: nextStaff,
    vars: mergeStaffIntoVariables(vars, nextStaff),
  };
}
