 ...
 * Deltas are directional until a CPA calibrates them.
 */
...
function snapshot(
  ...
  const result = runPrecogScenario(scoped, scenario.id, {
    staff,
    riskVariables: vars,
  })!;
  const portfolio = portfolioSummary(scoped, vars);
...
export function simulateCascadeLever(
  ...
  const rankedScenario =
    scenarioId ||
    state.scenarios.find((s) => s.id.includes("cash"))?.id ||
    state.scenarios[0]?.id ||
    "";

  const before = snapshot(state, baseVars, state.staff, rankedScenario);
  const applied = applyLever(lever.id, baseVars, state.staff);
  const after = snapshot(state, applied.vars, applied.staff, rankedScenario);
...
export function simulateAllCascades(
  ...
  const sid =
    ...
    state.scenarios[0]?.id ||
    "";
  const baseline = snapshot(state, baseVars, state.staff, sid);
  const simulations = CASCADE_LEVERS.map((l) =>
    simulateCascadeLever(state, l.id, baseVars, sid),
  );
