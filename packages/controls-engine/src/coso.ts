...
export interface CosoComponentAssessment {
  ...
  shortName: string;
  description: string;
  score: number; // 0-100, directional
  status: HealthStatus;
...
export function assessCoso(state: PracticeState): {
  overall: number;
  ...
  const risks = findKnowledgeRisks(state);
  const ranked = rankDangerousScenarios(state);
  const spofs = risks.filter((r) => r.soleOwner && r.riskScore >= 65);
  const sodGaps = state.controls.filter((c) => !c.segregated);
  ...
  const controlEnvScore = clamp100(
    Math.max(
      25,
      72 -
        (staff.segregationScore < 50 ? 12 : 0) -
        (unaddressedGaps.length > 2 ? 10 : 0),
    ),
  );

  const riskAssessmentScore = clamp100(
    Math.max(
      20,
      78 -
        spofs.length * 8 -
  ...
  const controlActivitiesScore = clamp100(
    Math.max(
      15,
  ...
  const monitoringScore = clamp100(
    Math.max(
      20,
      55 +
    ...
    },
    {
      ...
      principles: [
        ...
        },
        {
          ...
          name: "Fraud risk",
          status:
            !staff.dualControlPayments || !staff.independentBankRec
              ? "weak"
              : "adequate",
      ...
      findings: [
        ...
        },
        {
          ...
          detail: `${sodGaps.length} SoD gap(s); dual payment control ${staff.dualControlPayments ? "on" : "off"}; independent bank rec ${staff.independentBankRec ? "on" : "off"}.`,
          severity: "weak",
          link: { type: "sod" },
        },
      ],
      ...
      principles: [
        {
          ...
          status: statusFromScore(controlActivitiesScore),
          note: `Segregation score ${staff.segregationScore}/100 with ${sodGaps.length} active conflicts.`,
        },
        {
          number: 11,
