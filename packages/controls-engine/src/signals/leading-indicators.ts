 ...
 * Directional until a CPA calibrates thresholds. Never scores people.
 */
...
export function scoreLeadingIndicators(
  ...
  const { staff, vars } = alignedStaffAndVariables(state.staff, riskVars);
  const scoped = withStaff(state, staff);
  const portfolio = portfolioSummary(scoped, vars);
  const coso = assessCoso(scoped);
  const spofs = findKnowledgeRisks(scoped).filter(
    (r) => r.soleOwner && r.riskScore >= 65,
  );
  const openSod = scoped.controls.filter(
  ...
  const indicators: LeadingIndicator[] = [
    ...
    },
    {
      ...
      value: vars.claimsLoadFactor,
      threshold: 1.15,
      status:
        vars.claimsLoadFactor >= 1.3
          ? "breach"
          : vars.claimsLoadFactor >= 1.15
            ? "watch"
            : "ok",
      weight: 0.7,
    ...
    },
    {
      ...
      value: vars.dailyCashExposure,
      threshold: 4000,
      status:
        vars.dailyCashExposure >= 6000
          ? "breach"
          : vars.dailyCashExposure >= 4000
            ? "watch"
            : "ok",
      weight: 0.6,
