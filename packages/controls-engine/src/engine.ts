...
export function findKnowledgeRisks(state: PracticeState): KnowledgeRisk[] {
  const byK = new Map<string, typeof state.relations>();
  ...
  return state.knowledge
    ...
    .map((k) => {
      ...
      const owners = holders
        .map((h) => state.people.find((p) => p.id === h.personId))
...
function fraudMultiplier(
  ...
  return fraudRelated ? 1 + crimeStats.industryEmbezzlementRate * 0.5 : 1;
}
...
export function runPrecogScenario(
  ...
  const { staff, vars } = alignedStaffAndVariables(
    options?.staff ?? state.staff,
    options?.riskVariables,
  );

  const sMult = staffRiskMultiplier(staff);
  const fMult = fraudMultiplier(scenario, state.crimeStats);
  ...
  if (fMult > 1) {
    crimeModifiers.push(
      `Industry-oriented small-entity fraud base rate ~${Math.round(state.crimeStats.industryEmbezzlementRate * 100)}% annual exposure class (illustrative).`,
    );
    crimeModifiers.push(
      `Literature median detection lag ~${state.crimeStats.medianDetectionDays} days; p95 ~${state.crimeStats.detectionDaysP95} days when controls are weak.`,
    );
    crimeModifiers.push(
      `Typical mid-case loss reference ~$${state.crimeStats.typicalLossMid.toLocaleString()} (demo calibration, not a prediction of this practice).`,
    );
  } else {
    crimeModifiers.push(
  ...
  return {
    ...
    sources: [...scenario.statSources, state.crimeStats.source],
    assumptions: [
...
export function rankDangerousScenarios(
  ...
  const { staff } = alignedStaffAndVariables(
    options?.staff ?? state.staff,
    options?.riskVariables,
  );
  const scoped = withStaff(state, staff);
  return scoped.scenarios
    .map((scenario) => {
      const result = runPrecogScenario(scoped, scenario.id, options)!;
      ...
      const score =
        ...
        (staff.segregationScore < 50 ? 1.3 : 1);
