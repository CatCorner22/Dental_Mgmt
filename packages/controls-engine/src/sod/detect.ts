 ...
 *
 * Scores are directional until a CPA calibrates them.
 * Never score people — score incompatible duty combinations (control design).
 */
...
export function detectSodConflicts(
  ...
  const assignments = options?.assignments ?? buildAssignments(state.people);
  const residualAccepted =
    options?.residualAcceptedControlIds ??
    new Set(state.controls.filter((c) => c.residualRiskAccepted).map((c) => c.id));
  const compensatingByControl =
    options?.compensatingByControlId ??
    Object.fromEntries(
      state.controls
        .filter((c) => c.compensatingControls.length > 0)
        .map((c) => [c.id, c.compensatingControls]),
    );
