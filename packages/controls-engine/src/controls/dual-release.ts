 ...
 * waive_dual / force_dual are booleans only; numeric thresholds stay finite.
 */
...
export interface AppliedException {
  ...
  /** Always a finite USD amount — never Infinity or a sentinel. */
...
export interface ReleaseEvaluation {
  ...
  channel: ReleaseChannel;
  amountUsd: number;
  /** Always a finite number (base or raised/lowered). Never Infinity. */
  thresholdUsd: number;
  baseThresholdUsd: number;
...
/** Seed exceptions (owner-approved recurring lab payee + temporary raise). */
...
export function matchExceptions(
  ...
  const initiator = personById(people, request.initiatorPersonId);
...
/**
 * Resolve the numeric threshold and boolean flags.
 * thresholdUsd / effectiveThresholdUsd are always finite base-or-override amounts.
 * waiveDual / forceDual never leak sentinels into JSON or math.
 */
export function resolveEffectiveThreshold(
  ...
  if (exception.action === "waive_dual") {
    return {
      ...
      applied: {
        ...
        baseThresholdUsd,
        effectiveThresholdUsd: baseThresholdUsd,
  ...
  const thresholdUsd =
    exception.action === "raise_threshold"
      ...
      : Math.min(baseThresholdUsd, override);
...
export function evaluateRelease(
  ...
  const second = request.secondPersonId
    ? personById(people, request.secondPersonId)
    : undefined;
  ...
  if (!rule || !rule.enabled) {
    return {
      ...
      eligibleSeconds: listEligibleApprovers(policy, request.channel, people).filter(
        (p) => p.canSecond,
      ),
      initiator: initiator
        ? { id: initiator.id, name: initiator.name, role: initiator.role }
  ...
  const dualRequired = resolved.forceDual
    ...
    : resolved.waiveDual
      ? false
      : request.amountUsd > resolved.thresholdUsd;

  const eligible = listEligibleApprovers(policy, request.channel, people);
  ...
  if (!initiator) {
    return {
      ...
      channel: request.channel,
      amountUsd: request.amountUsd,
      thresholdUsd: reportedThreshold,
    ...
    return {
      ...
      reasons: [
        viaRaise
          ...
          : `Amount $${request.amountUsd.toLocaleString()} is at or under threshold $${reportedThreshold.toLocaleString()} — single release allowed.`,
    ...
    return {
      ...
      reasons: [
        resolved.forceDual
          ...
          : `Dual release required above $${reportedThreshold.toLocaleString()}.`,
        "Second signer not yet attached.",
        ...(resolved.applied &&
        resolved.applied.action !== "force_dual" &&
        resolved.applied.baseThresholdUsd !== reportedThreshold
          ? [
              `Base threshold $${rule.thresholdUsd.toLocaleString()} → effective $${reportedThreshold.toLocaleString()}.`,
            ]
          : []),
      ],
      nextSteps: [
  ...
  return {
    ...
    reasons: [
      ...
      resolved.forceDual
        ? `Channel ${rule.label} requires dual release (force_dual).`
        : `Channel ${rule.label} above effective threshold $${reportedThreshold.toLocaleString()}.`,
      ...(resolved.applied
